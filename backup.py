...
#store the column names in a dictionary with keys 0-7. New** from the ones starting with Old**
        # col_n = df.columns
        # i = 0
        # for names in col_n: 

        #   if i in range(len(col_n)):
                
        #       dict1[i] = names
        #       i = i+1
        # #rename the columns for the dataframe df with the dictionary keys
        
        # df.columns = dict1.keys()

        # for i in range(3, len(dict1.keys())+1):


        # #store the change in subject_i scores in arr1 
        #   np.arr1  = df[i] - df[i+1]
